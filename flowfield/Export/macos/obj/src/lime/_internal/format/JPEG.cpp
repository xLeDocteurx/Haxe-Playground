...
STATIC_HX_DEFINE_DYNAMIC_FUNC2(JPEG_obj,decodeFile,return )
 ...
 ::haxe::io::Bytes JPEG_obj::encode( ::lime::graphics::Image image,int quality){
            	HX_GC_STACKFRAME(&_hx_pos_4ccd4a1491a42735_80_encode)
...
HXLINE( 110)				 ::haxe::io::Bytes buffer = image->buffer->data->buffer;
HXLINE( 112)				 ::Dynamic data =  ::Dynamic(::hx::Anon_obj::Create(4)
            					->setFixed(0,HX_("pixels",2d,ef,a9,8c),buffer)
            					->setFixed(1,HX_("width",06,b6,62,ca),image->width)
            					->setFixed(2,HX_("height",e7,07,4c,02),image->height)
            					->setFixed(3,HX_("quality",bf,04,4c,44),( (Float)(quality) )));
HXLINE( 120)				 ::haxe::io::BytesOutput output =  ::haxe::io::BytesOutput_obj::__alloc( HX_CTX );
HXLINE( 121)				 ::format::jpg::Writer jpeg =  ::format::jpg::Writer_obj::__alloc( HX_CTX ,output);
HXLINE( 122)				jpeg->write(data);
HXLINE( 124)				return output->getBytes();
            			} catch( ::Dynamic _hx_e) {
