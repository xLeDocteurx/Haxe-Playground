...
class Main extends Application {
	var heapsApp:H2D_intro = new H2D_intro();

	public function new() {
		super();

		this.heapsApp.setCurrent();
	}
}
...
class H2D_intro extends hxd.App {
	...
	// var particlesQuantity = 1600;
	var particlesAlpha = 0.005;
	// var particlesAlpha = 0.0025;
	...
	function updateParticles() {
		for (i in 0...particles.length) {
			...
			this.particles[i].draw(this.customGraphics, float2color(0.188235294, 0.835294118, 0.7843));
		}
	}
...
class Particle {
	...
	public function applyForce(noiseValue:Float, magnitude:Float, windowWidth:Int, windowHeight:Int) {
		// public function applyForce(cell:Cell, magnitude:Float, windowWidth:Int, windowHeight:Int) {
		// var noiseValue = cell.noiseValue;
...
function isValidNoiseInput(input:Float):Bool {
